...
import com.consonance.invitation.controller.FriendInfoController;
...
import com.consonance.invitation.widget.FriendInfoView;
...
public class FriendInfoActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mTargetId = getIntent().getStringExtra(InApplication.TARGET_ID);
        mTargetAppKey = getIntent().getStringExtra(InApplication.TARGET_APP_KEY);
        if (mTargetAppKey == null) {
            mTargetAppKey = JMessageClient.getMyInfo().getAppKey();
        }
        mGroupId = getIntent().getLongExtra(InApplication.GROUP_ID, 0);
        Conversation conv;
        if (mGroupId == 0) {
    ...
    public void startChatActivity() {
        if (mGroupId != 0) {
            ...
            intent.putExtra(InApplication.NICKNAME, mNickname);
            setResult(InApplication.RESULT_CODE_FRIEND_INFO, intent);
        }
